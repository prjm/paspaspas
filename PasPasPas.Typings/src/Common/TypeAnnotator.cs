                    ...
                    else if ((symRef.Kind == SymbolReferencePartKind.CallParameters || symRef.Kind == SymbolReferencePartKind.StringCast) && symRef.Name != null)
                    {
                        ...
                        if (callableRoutines.Count == 1)
                        {
                            if (callableRoutines[0].ResultType.IsConstant()) {
                                ...
                                var fakeRoutine = new Routine(TypeRegistry, callableRoutines[0].Routine.Name, callableRoutines[0].Routine.Kind);
                                fakeRoutine.AddParameterGroup(callableRoutines[0].ResultType);
                                baseTypeValue = Runtime.Types.MakeInvocationResult(fakeRoutine,0);
                            }
                            else
                            {
